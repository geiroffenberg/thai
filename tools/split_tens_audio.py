#!/usr/bin/env python3
"""Split yisip_to_khaosip.mp3 into eight Thai tens number files.

Output files match the names expected by numbers.js:
  audio/numbers/20-yii-sip.mp3
  audio/numbers/30-saam-sip.mp3
  audio/numbers/40-sii-sip.mp3
  audio/numbers/50-haa-sip.mp3
  audio/numbers/60-hok-sip.mp3
  audio/numbers/70-jet-sip.mp3
  audio/numbers/80-paet-sip.mp3
  audio/numbers/90-gao-sip.mp3
"""
...
SOURCE = ROOT / "audio" / "numbers" / "yisip_to_khaosip.mp3"
OUTPUT_DIR = ROOT / "audio" / "numbers"

# Exactly 8 entries — must match the spoken order on the track.
TENS = [
    ("20", "yii-sip"),     # ยี่สิบ
    ("30", "saam-sip"),    # สามสิบ
    ("40", "sii-sip"),     # สี่สิบ
...
# Merge tuning for accidental intra-word splits (e.g., "hok" + "sip").
MERGE_MAX_GAP = 0.22
MERGE_TINY_SEGMENT = 0.28

# Extra audio around each word.
LEAD_PAD = 0.05
TAIL_PAD = 0.08

# Manual cut points if needed.
MANUAL_CUTS = []
...
def merge_micro_segments(segments: list[tuple[float, float]], target_count: int) -> list[tuple[float, float]]:
    """Merge adjacent tiny segments separated by a very short gap."""
    updated = list(segments)
    while len(updated) > target_count:
        merged = False
        for index in range(len(updated) - 1):
            left_start, left_end = updated[index]
            right_start, right_end = updated[index + 1]
            left_dur = left_end - left_start
            right_dur = right_end - right_start
            gap = right_start - left_end

            if gap <= MERGE_MAX_GAP and (left_dur < MERGE_TINY_SEGMENT or right_dur < MERGE_TINY_SEGMENT):
                updated[index:index + 2] = [(left_start, right_end)]
                print(
                    f"  Merged segments {index + 1} and {index + 2} "
                    f"(gap={gap:.3f}s, durs={left_dur:.3f}s/{right_dur:.3f}s)"
                )
                merged = True
                break
...
def main() -> None:
    ...
    if len(segments) > len(TENS):
        segments = merge_micro_segments(segments, len(TENS))
        print(f"After merge pass: {len(segments)} segment(s)")
